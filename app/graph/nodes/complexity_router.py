...
ROUTER_PROMPT = """Classify this lead message into exactly one category.
...
Respond with ONLY a JSON: {"complexity": "simple" | "complex", "reason": "brief reason"}"""
...
async def complexity_router_node(state: AgentState) -> dict:
    ...
    try:
        ...
        logger.info("Router decision: %s — %s", complexity, parsed.get("reason", ""))
        return {
